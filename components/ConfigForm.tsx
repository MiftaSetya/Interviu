  ...
  return (
    <div className="w-full max-w-md mx-auto bg-surface p-8 rounded-2xl shadow-xl border border-slate-700">
      <div className="mb-8 text-center">
        ...
        <p className="text-slate-400">Configure your AI practice partner</p>
      </div>
      ...
      <div className="flex gap-4 mb-6">
        ...
        <button
          ...
          className={`flex-1 flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${
            type === InterviewType.SCHOLARSHIP
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">
            {type === InterviewType.JOB ? 'Role / Position' : 'Scholarship Name'}
          </label>
          <input
            required
            type="text"
            ...
            placeholder={type === InterviewType.JOB ? "e.g., Senior Frontend Engineer" : "e.g., Chevening Scholarship"}
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">
            {type === InterviewType.JOB ? 'Company / Organization' : 'Target University / Country'}
          </label>
          <input
            required
            type="text"
            ...
            placeholder={type === InterviewType.JOB ? "e.g., Google" : "e.g., University of Oxford"}
            className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
          />
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">
            {type === InterviewType.JOB ? 'Experience Level (Optional)' : 'Academic Background (Optional)'}
          </label>
          <input
            type="text"
            value={experience}
            onChange={(e) => setExperience(e.target.value)}
            placeholder={type === InterviewType.JOB ? "e.g., 5 years, Lead" : "e.g., Undergraduate in Law"}
            className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
          />
        </div>
        ...
        <div>
           ...
           <input
            ...
            placeholder="e.g., Behavioral questions, Technical deep dive, Leadership"
            className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
          />
        </div>
